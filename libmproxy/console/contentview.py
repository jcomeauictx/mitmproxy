...
class ViewXML:
    ...
    def __call__(self, hdrs, content, limit):
        ...
        logging.debug('ViewXML: docinfo=%s', docinfo)
        prev = []
        ...
        while p is not None:
            ...
            p = p.getprevious()
        doctype = docinfo.doctype.encode()
        logging.debug('doctype: %r', doctype)
        if prev:
            logging.debug('ViewXML.__call__: prev=%r', prev)
            doctype += b'\n'.join(prev).strip()
